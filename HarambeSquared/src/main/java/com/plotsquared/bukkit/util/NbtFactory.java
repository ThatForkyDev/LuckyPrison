...
public final class NbtFactory {
	...
	public static NbtCompound fromStream(ByteSource stream, StreamOptions option) throws IOException
	{
		...
		try
		{
			...
			data = option == StreamOptions.GZIP_COMPRESSION ? new DataInputStream(new BufferedInputStream(new GZIPInputStream(input))) : new DataInputStream(new BufferedInputStream(input));
	...
	private class ConvertedMap extends AbstractMap<String, Object> implements Wrapper {
		...
		{
			Iterator<Map.Entry<String, Object>> proxy = this.original.entrySet().iterator();
			...
			return new Iterator<Map.Entry<String, Object>>() {
				...
				{
					Map.Entry<String, Object> entry = proxy.next();

					return new AbstractMap.SimpleEntry<>(entry.getKey(), ConvertedMap.this.wrapOutgoing(entry.getValue()));
				}
