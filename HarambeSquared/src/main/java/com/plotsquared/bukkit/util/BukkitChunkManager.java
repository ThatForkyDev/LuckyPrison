...
public class BukkitChunkManager extends ChunkManager {
	...
	public boolean copyRegion(Location pos1, Location pos2, Location newPos, Runnable whenDone)
	{
		...
		RegionWrapper region = new RegionWrapper(pos1.getX(), pos2.getX(), pos1.getZ(), pos2.getZ());
		World oldWorld = Bukkit.getWorld(pos1.getWorld());
		World newWorld = Bukkit.getWorld(newPos.getWorld());
		String newWorldName = newWorld.getName();
		List<ChunkLoc> chunks = new ArrayList<>();
		ContentMap map = new ContentMap();
		LocalBlockQueue queue = GlobalBlockQueue.IMP.getNewQueue(newWorldName, false);
				...
				for (int x = bx & 15; x <= (tx & 15); x++)
				{
					for (int z = bz & 15; z <= (tz & 15); z++)
					{
						map.saveBlocks(oldWorld, 256, cxx + x, czz + z, relX, relZ, true);
							   ...
							   {
								   for (Map.Entry<PlotLoc, PlotBlock[]> entry : map.allBlocks.entrySet())
								   {
	...
	public void swap(Location bot1, Location top1, Location bot2, Location top2, Runnable whenDone)
	{
		...
		ArrayDeque<ContentMap> maps = new ArrayDeque<>();
	...
	public static class ContentMap {
		...
		public void saveRegion(World world, int x1, int x2, int z1, int z2)
		{
			...
			for (int x = x1; x <= x2; x++)
			{
				for (int z = z1; z <= z2; z++)
				{
					this.saveBlocks(world, 256, x, z, 0, 0, true);
				}
			}
		}
		...
		{
			this.saveEntitiesIn(chunk, region, 0, 0, false);
		}
		...
		public void restoreEntities(World world, int xOffset, int zOffset)
		{
			for (EntityWrapper entity : this.entities)
			{
				...
				{
					PS.debug("Failed to restore entity (e): " + e);
					e.printStackTrace();
				}
			}
			this.entities.clear();
		...
		{
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.chestContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, String[]> blockLocEntry : this.signContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.dispenserContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.dropperContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.beaconContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, Material> blockLocMaterialEntry : this.jukeboxDisc.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, Object[]> blockLocEntry : this.skullData.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.hopperContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, Note> blockLocNoteEntry : this.noteBlockContents.entrySet())
			{
				try
				{
			...
			for (Map.Entry<BlockLoc, Short> blockLocShortEntry : this.brewTime.entrySet())
			{
				try
				{
					Block block = world.getBlockAt(
			...
			for (Map.Entry<BlockLoc, EntityType> blockLocEntityTypeEntry : this.spawnerData.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, String> blockLocStringEntry : this.cmdData.entrySet())
			{
				try
				{
					Block block = world.getBlockAt(
			...
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.brewingStandContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, Short[]> blockLocEntry : this.furnaceTime.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, ItemStack[]> blockLocEntry : this.furnaceContents.entrySet())
			{
				try
				{
					Block block =
			...
			for (Map.Entry<BlockLoc, DyeColor> blockLocByteEntry : this.bannerBase.entrySet())
			{
				try
				{
					Block block = world.getBlockAt(
		...
		public void saveBlocks(World world, int maxY, int x, int z, int offsetX, int offsetZ, boolean storeNormal)
		{
			...
			for (short y = 0; y <= maxY; y++)
			{
				...
				{
					int typeId = id.getId();
					ids[y] = typeId == 0 ? PlotBlock.EVERYTHING : PlotBlock.get((short) typeId, block.getData());
				}
				if (id != Material.AIR)
				{
					try
