...
public class BukkitLocalQueue<T> extends BasicLocalBlockQueue<T> {
	...
	{
		return (LocalChunk<T>) new BasicLocalBlockQueue.BasicLocalChunk(this, x, z) {
			// Custom stuff?
		};
	}
	...
	{
		World worldObj = Bukkit.getWorld(this.getWorld());
	...
	protected Object[] disableLighting(Chunk chunk)
	{
		try
		{
			...
			{
				this.chunkGetHandle = chunk.getClass().getDeclaredMethod("getHandle");
				this.chunkGetHandle.setAccessible(true);
			}
			Object nmsChunk = this.chunkGetHandle.invoke(chunk);
	...
	protected void enableLighting(Object[] disableResult)
	{
		if (disableResult != null)
		{
			try
			{
				this.fieldNeighbors.set(disableResult[0], 0x739C0);
			}
			catch (Throwable ignore) {}
		}
	}
