...
public class BukkitPlotGenerator extends ChunkGenerator implements GeneratorWrapper<ChunkGenerator> {
		...
		this.populators.add(new BlockPopulator() {
			...
			public void populate(World world, Random r, Chunk c)
			{
				...
				if (!BukkitPlotGenerator.this.dataMap.containsKey(loc))
				{
					...
					BukkitPlotGenerator.this.generate(world, loc.x, loc.z, result);
				...
				ScopedLocalBlockQueue chunk = this.queue.getForChunk(wrap.x, wrap.z);
	...
	public BukkitPlotGenerator(String world, ChunkGenerator cg)
	{
		...
		this.plotGenerator = new IndependentPlotGenerator() {
			...
			public void generateChunk(ScopedLocalBlockQueue result, PlotArea settings, PseudoRandom random)
			{
				...
				try
				{
					...
					ChunkGenerator.ChunkData data = cg.generateChunkData(w, r, cx, cz, grid);
					if (data != null)
					{
						return;
					}
	...
	public List<BlockPopulator> getDefaultPopulators(World world)
	{
		...
		{
			this.populators = new ArrayList<>();
			this.populators.addAll(this.platformGenerator.getDefaultPopulators(world));
		}
	...
	@Override
	public ChunkGenerator.ChunkData generateChunkData(World world, Random random, int cx, int cz, ChunkGenerator.BiomeGrid grid)
	{
		GenChunk result = this.chunkSetter;
		// Set the chunk location
		result.setChunk(new ChunkWrapper(world.getName(), cx, cz));
		// Set the result data
		result.cd = this.createChunkData(world);
		result.grid = grid;
		result.result = null;
		...
		try
		{
			...
			{
				this.generate(world, cx, cz, result);
			}
		}
		catch (Throwable e)
		{
	...
	@Override
	public short[][] generateExtBlockSections(World world, Random r, int cx, int cz, ChunkGenerator.BiomeGrid grid)
	{
		GenChunk result = this.chunkSetter;
		// Set the chunk location
		result.setChunk(new ChunkWrapper(world.getName(), cx, cz));
	...
	public String toString()
	{
		...
		return this.platformGenerator == null ? "null" : this.platformGenerator.getClass().getName();
	}
