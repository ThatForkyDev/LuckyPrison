...
public class PlotListener {
	...
	public static boolean plotEntry(PlotPlayer player, Plot plot)
	{
					...
					if (player.getGameMode() != gamemodeFlag.get())
					{
						...
						else
						{
							MainUtil.sendMessage(player,
												 StringMan.replaceAll(C.GAMEMODE_WAS_BYPASSED.s(),
																	  "{plot}", plot.getId(),
					...
					TaskManager.runTaskLaterAsync(() ->
												  {
													  ...
													  if ((lastPlot != null) && plot.getId().equals(lastPlot.getId()))
													  {
														  ...
														  replacements.put("%z%", String.valueOf(lastPlot.getId().y));
	...
	public static boolean plotExit(PlotPlayer player, Plot plot)
	{
			...
			if (leave.isPresent() && leave.get())
			{
				if (!Permissions.hasPermission(player, "plots.flag.notify-enter.bypass"))
				{
					for (UUID uuid : plot.getOwners())
					{
						...
						if ((owner != null) && !owner.getUUID().equals(player.getUUID()))
						{
							MainUtil.sendMessage(owner, C.NOTIFY_LEAVE.s()
																	  .replace("%player", player.getName())
