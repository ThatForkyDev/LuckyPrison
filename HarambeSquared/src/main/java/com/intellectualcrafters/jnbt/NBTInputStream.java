...
public final class NBTInputStream implements Closeable {
	...
	{
		return this.readTag(0, Integer.MAX_VALUE);
	}
	...
	{
		return this.readTag(0, maxDepth);
	}
	...
	private Tag readTag(int depth, int maxDepth) throws IOException
	{
		...
		return this.readTagPayload(type, name, depth, maxDepth);
	}
	...
	private Tag readTagPayload(int type, String name, int depth, int maxDepth) throws IOException
	{
		...
		switch (type)
		{
			...
			case NBTConstants.TYPE_LIST:
				...
				{
					Tag tag = this.readTagPayload(childType, "", depth + 1, maxDepth);
			...
			case NBTConstants.TYPE_COMPOUND:
				...
				while (true)
				{
					Tag tag = this.readTag(depth + 1, maxDepth);
					if (tag instanceof EndTag)
					{
						break;
					}
