		...
		for (int i = 0; i < assets.length(); i++)
		{
			...
			if (downloadURL.equals(name))
			{
				try
				{
					...
					int[] version = split.length == 3 ? new int[]{Integer.parseInt(split[0]), Integer.parseInt(split[1]), Integer.parseInt(split[2])} : new int[]{Integer.parseInt(split[0]), Integer.parseInt(split[1]), 0};
