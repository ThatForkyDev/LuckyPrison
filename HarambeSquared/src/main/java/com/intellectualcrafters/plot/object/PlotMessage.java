...
public class PlotMessage {
	...
	public PlotMessage()
	{
		try
		{
			this.reset(ChatManager.manager);
		}
		catch (Throwable e)
		{
			PS.debug("PlotSquared doesn't support fancy chat for " + Arrays.toString(PS.get().IMP.getServerVersion()));
	...
	{
		return this.tooltip(new PlotMessage(tooltip));
	}
