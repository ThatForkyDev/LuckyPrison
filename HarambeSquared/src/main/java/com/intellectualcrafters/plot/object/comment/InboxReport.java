...
public class InboxReport implements CommentInbox {
	...
	public boolean canRead(Plot plot, PlotPlayer player)
	{
		if (Permissions.hasPermission(player, "plots.inbox.read." + this))
		{
			if (plot.isOwner(player.getUUID()) || Permissions
														  .hasPermission(player, "plots.inbox.read." + this + ".other"))
			{
				return true;
			}
		}
	...
	{
		DBFunc.getComments(null, this.toString(), new RunnableVal<List<PlotComment>>() {
			@Override
