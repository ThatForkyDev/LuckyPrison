...
public class Plot {
	...
	{
		return this.area.getCluster(this.id);
	}
	...
	public boolean isAdded(UUID uuid)
	{
		...
		if (this.getMembers().contains(uuid) || this.getMembers().contains(DBFunc.everyone))
		{
			if (this.isOnline())
			{
				return true;
	...
	public void setDenied(Set<UUID> uuids)
	{
		...
		HashSet<UUID> intersection = new HashSet<>(larger ? this.getDenied() : uuids);
		intersection.retainAll(larger ? uuids : this.getDenied());
	...
	public void setTrusted(Set<UUID> uuids)
	{
		...
		HashSet<UUID> intersection = new HashSet<>(larger ? this.getTrusted() : uuids);
	...
	public void setMembers(Set<UUID> uuids)
	{
		...
		HashSet<UUID> intersection = new HashSet<>(larger ? this.getMembers() : uuids);
	...
	public boolean clear(boolean checkRunning, boolean isDelete, Runnable whenDone)
	{
		...
		if (!this.isMerged() && this.area.getRegion().equals(this.getLargestRegion()))
		{
			LocalBlockQueue blockQueue = this.area.getQueue(false);
		...
		{
			ArrayDeque<Plot> queue = new ArrayDeque<>(plots);
	...
	{
		ArrayDeque<RegionWrapper> regions = new ArrayDeque<>(this.getRegions());
		Runnable run = new Runnable() {
			@Override
			public void run()
			{
	...
	public void setSign(String name)
	{
		...
		{
			TaskManager.runTask(() -> this.setSign(name));
			return;
		}
		PlotManager manager = this.area.getPlotManager();
	 ...
	 */
	public boolean deletePlot(Runnable whenDone)
	{
		if (!this.hasOwner())
		{
			return false;
	...
	public Location getCenter()
	{
		...
		loc.setY(1 + Math.max(WorldUtil.IMP.getHighestBlock(this.area.worldname, loc.getX(), loc.getZ()),
							  this.getManager().getSignLoc(this.area, this).getY()));
		return loc;
	}
	...
	public Location getSide()
	{
		...
		int y = Math.max(WorldUtil.IMP.getHighestBlock(this.area.worldname, x, z), manager.getSignLoc(this.area, this).getY());
		return new Location(this.area.worldname, x, y + 1, z);
	}
	...
	{
		LocalBlockQueue queue = GlobalBlockQueue.IMP.getNewQueue(this.area.worldname, false);
	 ...
	 * @return true if plot was created successfully
	 */
	public boolean create(UUID uuid, boolean notify)
	{
		if (uuid == null)
		{
		...
		if (this.area.addPlot(this))
		{
			DBFunc.createPlotAndSettings(this, () ->
			{
				...
				{
					this.autoMerge(-1, Integer.MAX_VALUE, uuid, true);
				}
			});
			return true;
		}
	...
	public boolean swapData(Plot plot, Runnable whenDone)
	{
		...
		// Swap cached
		PlotId temp = new PlotId(this.id.x, this.id.y);
		this.id.x = plot.id.x;
		this.id.y = plot.id.y;
		plot.id.x = temp.x;
		plot.id.y = temp.y;
	...
	public PlotId[] getCornerIds()
	{
		if (!this.isMerged())
		{
			return new PlotId[]{this.id, this.id};
		}
		PlotId min = new PlotId(this.id.x, this.id.y);
		PlotId max = new PlotId(this.id.x, this.id.y);
		for (Plot current : this.getConnectedPlots())
		{
	...
	public boolean removeDenied(UUID uuid)
	{
		if (uuid == DBFunc.everyone)
		{
			...
			for (UUID other : new HashSet<>(this.getDenied()))
			{
	...
	public boolean removeTrusted(UUID uuid)
	{
		...
		{
			boolean result = false;
			for (UUID other : new HashSet<>(this.getTrusted()))
			{
	 ...
	 * @return
	 */
	public void export(RunnableVal<Boolean> whenDone)
	{
	 ...
	 */
	public void upload(RunnableVal<URL> whenDone)
	{
		SchematicHandler.manager.getCompoundTag(this, new RunnableVal<CompoundTag>() {
			@Override
			public void run(CompoundTag value)
		...
		try
		{
			...
			for (int i = 4; i > 0; i--)
			{
				...
				if (lines[i - 1].length() == 15)
				{
					...
					for (Map.Entry<StringWrapper, UUID> entry : map.entrySet())
					{
		...
		TaskManager.runTaskLater(() ->
								 {
									 ...
									 {
										 PlotListener.plotExit(pp, this);
										 PlotListener.plotEntry(pp, this);
									 }
								 }, 1);
	}
	...
	public boolean teleportPlayer(PlotPlayer player)
	{
		...
		if (result)
		{
			Location location = this.area.HOME_ALLOW_NONMEMBER || plot.isAdded(player.getUUID()) ? this.getHome() : this.getDefaultHome();
			...
			MainUtil.sendMessage(player, C.TELEPORT_IN_SECONDS, String.valueOf(Settings.Teleport.DELAY));
	...
	{
		if (StringMan.isEqualToAny(component, this.getManager().getPlotComponents(this.area, this.id)))
		{
			EventUtil.manager.callComponentSet(this, component);
		}
		return this.getManager().setComponent(this.area, this.id, component, blocks);
	}
	...
	public void updateWorldBorder()
	{
		...
		PlotId id = new PlotId(Math.abs(this.id.x) + 1, Math.abs(this.id.x) + 1);
		PlotManager manager = this.getManager();
	 ...
	 * @return
	 */
	public boolean move(Plot destination, Runnable whenDone, boolean allowSwap)
	{
		PlotId offset = new PlotId(destination.id.x - this.id.x, destination.id.y - this.id.y);
		Location db = destination.getBottomAbs();
		Location ob = this.getBottomAbs();
		int offsetX = db.getX() - ob.getX();
		int offsetZ = db.getZ() - ob.getZ();
		if (this.owner == null)
		{
			TaskManager.runTaskLater(whenDone, 1);
			return false;
		...
		for (Plot plot : plots)
		{
			Plot other = plot.getRelative(destination.area, offset.x, offset.y);
			if (other.hasOwner())
			{
				if (!allowSwap)
				{
	 ...
	 * @return
	 */
	public boolean copy(Plot destination, Runnable whenDone)
	{
		PlotId offset = new PlotId(destination.id.x - this.id.x, destination.id.y - this.id.y);
		Location db = destination.getBottomAbs();
		Location ob = this.getBottomAbs();
		...
		Runnable run = new Runnable() {
			...
			public void run()
			{
				if (regions.isEmpty())
				{
					for (Plot current : Plot.this.getConnectedPlots())
					{
