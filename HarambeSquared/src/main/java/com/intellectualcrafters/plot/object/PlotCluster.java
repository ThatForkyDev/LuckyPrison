...
public class PlotCluster {
	...
	public Location getHome()
	{
		...
		{
			// default pos
			Plot center = this.getCenterPlot();
		...
		{
			toReturn = this.getClusterBottom().add(home.x, home.y, home.z);
		}
	...
	{
		PlotId bot = this.pos1;
		PlotId top = this.pos2;
	...
	{
		PlotManager manager = this.area.getPlotManager();
		return manager.getPlotBottomLocAbs(this.area, this.pos1);
	}

	public Location getClusterTop()
	{
		PlotManager manager = this.area.getPlotManager();
		return manager.getPlotTopLocAbs(this.area, this.pos2);
	}
