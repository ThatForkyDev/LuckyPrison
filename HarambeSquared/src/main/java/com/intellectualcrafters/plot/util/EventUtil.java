...
public abstract class EventUtil {
	...
	public void doJoinTask(PlotPlayer player)
	{
	...
	public void doRespawnTask(PlotPlayer player)
	{
		Plot plot = player.getCurrentPlot();
	...
	public boolean checkPlayerBlockEvent(PlotPlayer player, PlayerBlockEventType type, Location location, LazyBlock block, boolean notifyPerms)
	{
		...
		Plot plot = area != null ? area.getPlot(location) : null;
		if (plot == null)
		{
			if (area == null)
			{
		...
		switch (type)
		{
			...
			case INTERACT_BLOCK:
			{
				...
				HashSet<PlotBlock> value = flagValue.isPresent() ? flagValue.get() : null;
				if (value == null || !value.contains(PlotBlock.EVERYTHING) && !value.contains(block.getPlotBlock()))
				{
					return Permissions.hasPermission(player, C.PERMISSION_ADMIN_INTERACT_OTHER.s(), false) ||
						   !(!notifyPerms || MainUtil.sendMessage(player, C.FLAG_TUTORIAL_USAGE, C.FLAG_USE.s()));
				}
			...
			case INTERACT_MISC:
			{
				...
				HashSet<PlotBlock> value = flag.isPresent() ? flag.get() : null;
				if (value == null || !value.contains(PlotBlock.EVERYTHING) && !value.contains(block.getPlotBlock()))
				{
					if (Permissions.hasPermission(player, C.PERMISSION_ADMIN_INTERACT_OTHER.s(), false))
					{
