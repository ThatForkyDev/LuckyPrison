...
public abstract class WorldUtil {
	...
	public void upload(Plot plot, UUID uuid, String file, RunnableVal<URL> whenDone)
	{
		if (plot == null)
		{
			throw new IllegalArgumentException("Plot may not be null!");
		}
		Location home = plot.getHome();
				...
				try (final ZipOutputStream zos = new ZipOutputStream(output))
				{
					...
					for (Plot current : plot.getConnectedPlots())
					{
						...
						for (int x = brx; x <= trx; x++)
						{
							...
							{
								File file = WorldUtil.this.getMcr(plot.getArea().worldname, x, z);
								if (file != null)
								{
