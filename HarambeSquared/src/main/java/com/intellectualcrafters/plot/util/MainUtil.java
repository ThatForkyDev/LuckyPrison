...
public final class MainUtil {
	 ...
	 */
	public static boolean canSendChunk;
	/**
	...
	public static void upload(UUID uuid, String file, String extension, RunnableVal<OutputStream> writeTask, RunnableVal<URL> whenDone)
	{
		if (writeTask == null)
		{
		...
		TaskManager.runTaskAsync(() ->
								 {
									 try
									 {
											  ...
											  PrintWriter writer = new PrintWriter(new OutputStreamWriter(output, StandardCharsets.UTF_8), true))
										 {
											 ...
											 writer.append("--").append(boundary).append(CRLF);
											 writer.append("Content-Disposition: form-data; name=\"schematicFile\"; filename=\"").append(filename).append('"').append(CRLF);
	 ...
	 * @param whenDone
	 */
	public static void format(String info, Plot plot, PlotPlayer player, boolean full, RunnableVal<String> whenDone)
	{
		...
		String description = descriptionFlag.isPresent() ? Flags.DESCRIPTION.valueToString(descriptionFlag.get()) : C.NONE.s();
			...
			TaskManager.runTaskAsync(() ->
									 {
										 ...
										 {
											 info1 = newInfo.replaceAll("%rating%", String.format("%.1f", plot.getAverageRating()) + '/' + max);
										 }
										 whenDone.run(info1);
									 });
			return;
