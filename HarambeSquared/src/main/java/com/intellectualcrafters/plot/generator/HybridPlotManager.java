...
public class HybridPlotManager extends ClassicPlotManager {
	...
	public boolean createRoadEast(PlotArea plotArea, Plot plot)
	{
		...
		Location bot = this.getPlotBottomLocAbs(hpw, id2);
		Location top = this.getPlotTopLocAbs(hpw, id);
		...
		this.createSchemAbs(hpw, queue, pos1, pos2, true);
		queue.enqueue();
		return true;
	}
	...
	private void createSchemAbs(HybridPlotWorld hpw, LocalBlockQueue queue, Location pos1, Location pos2, boolean clear)
	{
		...
		for (int x = pos1.getX(); x <= pos2.getX(); x++)
		{
			...
			for (int z = pos1.getZ(); z <= pos2.getZ(); z++)
			{
				...
				if (clear)
				{
					for (short y = 0; y <= hpw.SCHEMATIC_HEIGHT; y++)
					{
	...
	public boolean createRoadSouthEast(PlotArea plotArea, Plot plot)
	{
		...
		Location pos1 = this.getPlotTopLocAbs(hpw, id).add(1, 0, 1);
		Location pos2 = this.getPlotBottomLocAbs(hpw, id2);
		pos1.setY(0);
	...
	public boolean clearPlot(PlotArea plotArea, Plot plot, Runnable whenDone)
	{
		...
		// If augmented
		boolean canRegen = (plotArea.TYPE == 0) && (plotArea.TERRAIN == 0) && REGENERATIVE_CLEAR;
		// The component blocks
		PlotBlock[] plotfloor = dpw.TOP_BLOCK;
		PlotBlock[] filling = dpw.MAIN_BLOCK;
		PlotBlock bedrock = dpw.PLOT_BEDROCK ? PlotBlock.get((short) 7, (byte) 0) : PlotBlock.get((short) 0, (byte) 0);
		PlotBlock air = PlotBlock.get((short) 0, (byte) 0);
		String biome = dpw.PLOT_BIOME;
		LocalBlockQueue queue = plotArea.getQueue(false);
		String world = plotArea.worldname;
		ChunkManager.chunkTask(pos1, pos2, new RunnableVal<int[]>() {
			...
			public void run(int[] value)
			{
				...
				HybridPlotManager.this.pastePlotSchematic(dpw, queue, bot, top);
			}
		}, () ->
							   {
								   queue.enqueue();
	...
	public void pastePlotSchematic(HybridPlotWorld plotWorld, LocalBlockQueue queue, Location l1, Location l2)
	{
		...
		this.createSchemAbs(plotWorld, queue, l1, l2, false);
	}
}
