...
public class HybridPlotWorld extends ClassicPlotWorld {
	...
	public void loadConfiguration(ConfigurationSection config)
	{
		...
		this.PATH_WIDTH_LOWER = (this.ROAD_WIDTH & 1) == 0 ? (short) (Math.floor(this.ROAD_WIDTH / 2) - 1) : (short) Math.floor(this.ROAD_WIDTH / 2);
			...
			for (short x = 0; x < w3; x++)
			{
				for (short z = 0; z < l3; z++)
				{
					for (short y = 0; y < h3; y++)
					{
						...
						if (id != 0)
						{
							this.addOverlayBlock((short) (x + shift + oddshift + centerShiftX), (short) (y + this.PLOT_HEIGHT),
		...
		for (short x = 0; x < w1; x++)
		{
			for (short z = 0; z < l1; z++)
			{
				for (short y = 0; y < h1; y++)
				{
					...
					if (id != 0)
					{
						this.addOverlayBlock((short) (x - shift), (short) (y + this.ROAD_HEIGHT), (short) (z + shift + oddshift), id, data, false);
						this.addOverlayBlock((short) (z + shift + oddshift), (short) (y + this.ROAD_HEIGHT), (short) (x - shift), id, data, true);
					}
				}
			}
		}
		for (short x = 0; x < w2; x++)
		{
			for (short z = 0; z < l2; z++)
			{
				for (short y = 0; y < h2; y++)
				{
					...
					if (id != 0)
					{
						this.addOverlayBlock((short) (x - shift), (short) (y + this.ROAD_HEIGHT), (short) (z - shift), id, data, false);
					}
				}
			}
		}
