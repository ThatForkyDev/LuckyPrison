...
public class ClassicPlotManager extends SquarePlotManager {
	...
	public boolean setComponent(PlotArea plotArea, PlotId plotId, String component, PlotBlock[] blocks)
	{
		...
		{
			case "floor":
				this.setFloor(plotArea, plotId, blocks);
				return true;
			case "wall":
				this.setWallFilling(plotArea, plotId, blocks);
				return true;
			case "all":
				this.setAll(plotArea, plotId, blocks);
				return true;
			case "air":
				this.setAir(plotArea, plotId, blocks);
				return true;
			case "main":
				this.setMain(plotArea, plotId, blocks);
				return true;
			case "middle":
				this.setMiddle(plotArea, plotId, blocks);
				return true;
			case "outline":
				this.setOutline(plotArea, plotId, blocks);
				return true;
			case "border":
				this.setWall(plotArea, plotId, blocks);
				return true;
		}
		return false;
	}
	...
	public boolean unclaimPlot(PlotArea plotArea, Plot plot, Runnable whenDone)
	{
		...
		this.setWallFilling(dpw, plot.getId(), new PlotBlock[]{dpw.WALL_FILLING});
		if (dpw.WALL_BLOCK.id != 0 || !dpw.WALL_BLOCK.equals(dpw.CLAIMED_WALL_BLOCK))
		{
			this.setWall(dpw, plot.getId(), new PlotBlock[]{dpw.WALL_BLOCK});
		}
	...
	{
		ClassicPlotWorld dpw = (ClassicPlotWorld) plotArea;
		Location pos1 = this.getPlotBottomLocAbs(plotArea, plot.getId());
		Location pos2 = this.getPlotTopLocAbs(plotArea, plot.getId());
		int sx = pos2.getX() + 1;
	...
	{
		ClassicPlotWorld dpw = (ClassicPlotWorld) plotArea;
		Location location = this.getPlotTopLocAbs(dpw, plot.getId());
	...
	public boolean finishPlotMerge(PlotArea plotArea, ArrayList<PlotId> plotIds)
	{
		...
		if (block.id != 0 || !block.equals(unclaim))
		{
			...
			{
				this.setWall(plotArea, id, new PlotBlock[]{block});
			}
		}
		return true;
	}
	...
	public boolean claimPlot(PlotArea plotArea, Plot plot)
	{
		...
		{
			this.setWall(plotArea, plot.getId(), new PlotBlock[]{claim});
		}
		return true;
	}
