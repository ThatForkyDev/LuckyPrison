 ...
 */
public abstract class SquarePlotManager implements GridPlotManager {
	...
	public PlotId getPlotIdAbs(PlotArea plotArea, int x, int y, int z)
	{
		...
		{
			pathWidthLower = (dpw.ROAD_WIDTH % 2) == 0 ? (dpw.ROAD_WIDTH / 2) - 1 : dpw.ROAD_WIDTH / 2;
	...
	public PlotId getPlotId(PlotArea plotArea, int x, int y, int z)
	{
		try
		{
			...
			boolean[] merged = {rz <= pathWidthLower, rx > end, rz > end, rx <= pathWidthLower};
