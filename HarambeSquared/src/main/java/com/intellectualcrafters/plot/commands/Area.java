		...
		switch (args[0].toLowerCase())
		{
			...
			case "create":
				...
				switch (args.length)
				{
					...
					case 2:
						switch (args[1].toLowerCase())
						{
							...
							case "pos2":  // Set position 2 and finish creation for type=2 (partial)
								...
								RegionWrapper region = new RegionWrapper(bx, tx, bz, tz);
								...
								String path = "worlds." + area.worldname + ".areas." + area.id + '-' + object.min + '-' + object.max;
								Runnable run = () ->
								{
									if (offsetX != 0)
									{
					...
					default: // Start creation
						...
						HybridPlotWorld pa = new HybridPlotWorld(object.world, id, new HybridGen(), null, null);
						...
						for (int i = 2; i < args.length; i++)
						{
							...
							if (pair.length != 2)
							{
								C.COMMAND_SYNTAX.send(player, this.getCommandString() + " create [world[:id]] [<modifier>=<value>]...");
								return false;
							}
						...
						MainUtil.sendMessage(player, "$1Go to the first corner and use: $2 " + this.getCommandString() + " create pos1");
						break;
				}
				return true;
			case "i":
			case "info":
			{
				...
				switch (args.length)
				{
					...
					case 2:
						...
						break;
					default:
						C.COMMAND_SYNTAX.send(player, this.getCommandString() + " info [area]");
						return false;
				}
				if (area == null)
				{
				...
				{
					name = area.worldname;
					percent = claimed == 0 ? 0 : 100.0d * claimed / Integer.MAX_VALUE;
					region = "N/A";
				}
			...
			case "list":
				...
				switch (args.length)
				{
					...
					default:
						C.COMMAND_SYNTAX.send(player, this.getCommandString() + " list [#]");
						return false;
				}
				ArrayList<PlotArea> areas = new ArrayList<>(PS.get().getPlotAreas());
				this.paginate(player, areas, 8, page, new RunnableVal3<Integer, PlotArea, PlotMessage>() {
					@Override
					public void run(Integer i, PlotArea area, PlotMessage message)
					{
						...
						// type / terrain
						String visit = "/plot area tp " + area;
						message.text("[").color("$3")
							   .text(String.valueOf(i)).command(visit).tooltip(visit).color("$1")
							   .text("]").color("$3")
							   .text(' ' + name).tooltip(tooltip).command(Area.this.getCommandString() + " info " + area).color("$1").text(" - ")
							   .color("$2")
