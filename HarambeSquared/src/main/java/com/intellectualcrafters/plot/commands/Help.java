...
public class Help extends Command {
	...
	public void execute(PlotPlayer player, String[] args, RunnableVal3<Command, Runnable, Runnable> confirm, RunnableVal2<Command, CommandResult> whenDone)
	{
		...
		{
			case 0:
				this.displayHelp(player, null, 0);
				return;
			case 1:
				if (MathMan.isInteger(args[0]))
				{
					try
					{
						this.displayHelp(player, null, Integer.parseInt(args[0]));
					}
					catch (NumberFormatException ignored)
					{
						this.displayHelp(player, null, 1);
					}
				}
				else
				{
					this.displayHelp(player, args[0], 1);
				}
				return;
			case 2:
				if (MathMan.isInteger(args[1]))
				{
					try
					{
						this.displayHelp(player, args[1], Integer.parseInt(args[1]));
					}
					catch (NumberFormatException ignored)
					{
						this.displayHelp(player, args[1], 1);
					}
				}
				return;
			default:
	...
	public void displayHelp(PlotPlayer player, String cat, int page)
	{
		...
		if (cat == null && page == 0)
		{
			...
			for (CommandCategory c : CommandCategory.values())
			{
				builder.append('\n').append(StringMan.replaceAll(C.HELP_INFO_ITEM.s(), "%category%", c.toString().toLowerCase(), "%category_desc%", c.toString()));
			}
		...
		page--;
		new HelpMenu(player).setCategory(catEnum).getCommands().generateMaxPages().generatePage(page, this.getParent().toString()).render();
	}
}
