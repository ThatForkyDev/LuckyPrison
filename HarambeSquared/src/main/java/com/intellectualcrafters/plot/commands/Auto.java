...
public class Auto extends SubCommand {
	...
	public static PlotId getNextPlotId(PlotId id, int step)
	{
		...
		if (absX > absY)
		{
			return id.x > 0 ? new PlotId(id.x, id.y + 1) : new PlotId(id.x, id.y - 1);
		}
		else if (absY > absX)
		{
			return id.y > 0 ? new PlotId(id.x - 1, id.y) : new PlotId(id.x + 1, id.y);
		}
		else
		{
			if (id.x == id.y && id.x > 0)
	...
	public boolean onCommand(PlotPlayer player, String... args)
	{
		...
		{
			MainUtil.sendMessage(player, C.CANT_CLAIM_MORE_PLOTS_NUM, String.valueOf(Settings.Claim.MAX_AUTO_AREA));
			return false;
		}
		...
		if (diff + size_x * size_z > 0)
		{
			if (diff < 0)
			{
				MainUtil.sendMessage(player, C.CANT_CLAIM_MORE_PLOTS_NUM, String.valueOf(-diff));
				return false;
			}
			else if (player.hasPersistentMeta("grantedPlots"))
			{
				...
				else
				{
					...
					this.sendMessage(player, C.REMOVED_GRANTED_PLOT, String.valueOf(left), String.valueOf(grantedPlots - left));
				}
			}
			else
			{
		...
		if (schematic != null && !schematic.isEmpty())
		{
			...
			{
				this.sendMessage(player, C.SCHEMATIC_INVALID, "non-existent: " + schematic);
				return true;
			}
		...
		while (true)
		{
			PlotId start = getNextPlotId(this.getLastPlotId(plotarea), 1);
