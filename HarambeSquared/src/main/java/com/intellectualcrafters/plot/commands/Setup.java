...
public class Setup extends SubCommand {
	...
	public void displayGenerators(PlotPlayer player)
	{
		...
		{
			if ("PlotSquared".equals(entry.getKey()))
			{
	...
	public boolean onCommand(PlotPlayer player, String... args)
	{
		...
		if (object == null)
		{
			...
			this.sendMessage(player, C.SETUP_INIT);
		...
		if (args.length == 1)
		{
			...
			if ("back".equalsIgnoreCase(args[0]))
			{
				if (object.setup_index > 0)
				{
					...
					this.sendMessage(player, C.SETUP_STEP, object.setup_index + 1, node.getDescription(), node.getType().getType(),
		...
		switch (index)
		{
			...
			case 1:  // choose world type
				...
				if (object.type == 0)
				{
					...
					this.sendMessage(player, C.SETUP_STEP, object.setup_index + 1, step.getDescription(), step.getType().getType(),
			...
			case 6:  // world setup
				...
				if (valid)
				{
					this.sendMessage(player, C.SETUP_VALID_ARG, step.getConstant(), args[0]);
				...
				{
					this.sendMessage(player, C.SETUP_INVALID_ARG, args[0], step.getConstant());
					this.sendMessage(player, C.SETUP_STEP, object.setup_index + 1, step.getDescription(), step.getType().getType(),
									 String.valueOf(step.getDefaultValue()));
					return false;
				}
			case 7:
				...
				player.deleteMeta("setup");
				String world;
				world = object.setupManager == null ? SetupUtils.manager.setupWorld(object) : object.setupManager.setupWorld(object);
				try
				{
				...
				this.sendMessage(player, C.SETUP_FINISHED, object.world);
		}
		return false;
	}
}
