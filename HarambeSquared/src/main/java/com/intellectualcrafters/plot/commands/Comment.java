...
public class Comment extends SubCommand {
	...
	public boolean onCommand(PlotPlayer player, String... args)
	{
		if (args.length < 2)
		{
			this.sendMessage(player, C.COMMENT_SYNTAX, StringMan.join(CommentManager.inboxes.keySet(), "|"));
			return false;
		}
		...
		if (!result)
		{
			this.sendMessage(player, C.NO_PLOT_INBOX, "");
			this.sendMessage(player, C.COMMENT_SYNTAX, StringMan.join(CommentManager.inboxes.keySet(), "|"));
			return false;
		}
		...
		this.sendMessage(player, C.COMMENT_ADDED);
		return true;
	}
}
