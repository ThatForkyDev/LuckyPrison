...
@CommandDeclaration(command = "schematic", permission = "plots.schematic", description = "Schematic command", aliases = "sch", category = CommandCategory.SCHEMATIC, usage = "/plot schematic <arg...>")
public class SchematicCmd extends SubCommand {
	...
	public boolean onCommand(PlotPlayer player, String... args)
	{
		if (args.length < 1)
		{
			this.sendMessage(player, C.SCHEMATIC_MISSING_ARG);
			return true;
		}
		...
		switch (arg)
		{
				...
				TaskManager.runTaskAsync(() ->
										 {
											 ...
											 if (location.startsWith("url:"))
											 {
												 ...
												 {
													 e.printStackTrace();
													 this.sendMessage(player, C.SCHEMATIC_INVALID, "non-existent url: " + location);
													 this.running = false;
													 return;
												 }
											 }
											 ...
											 SchematicHandler.manager.paste(schematic, plot, 0, 0, 0, true, new RunnableVal<Boolean>() {
												 ...
												 public void run(Boolean value)
												 {
													 ...
													 if (value)
													 {
														 SchematicCmd.this.sendMessage(player, C.SCHEMATIC_PASTE_SUCCESS);
													 }
													 else
													 {
														 SchematicCmd.this.sendMessage(player, C.SCHEMATIC_PASTE_FAILED);
													 }
												 }
											 });
										 });
			...
			case "exportall":
			{
				...
				boolean result = SchematicHandler.manager.exportAll(plots, null, null, () -> MainUtil.sendMessage(player, "&aFinished mass export"));
				if (!result)
				{
					MainUtil.sendMessage(player, "&cTask is already running.");
					return false;
