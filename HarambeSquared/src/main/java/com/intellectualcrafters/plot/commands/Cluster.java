	...
	public boolean onCommand(PlotPlayer player, String... args)
	{
		...
		switch (sub)
		{
			...
			case "list":
			{
				...
				MainUtil.sendMessage(player, C.CLUSTER_LIST_HEADING, String.valueOf(clusters.size()));
			...
			case "create":
			{
				...
				int current = Settings.Limit.GLOBAL ? player.getPlayerClusterCount() : player.getPlayerClusterCount(player.getLocation().getWorld());
			...
			case "kick":
			{
				...
				for (Plot plot : new ArrayList<>(PS.get().getPlots(player2.getLocation().getWorld(), uuid)))
				{
					...
					if (Objects.equals(current, cluster))
					{
						plot.unclaim();
					}
				}
			...
			case "information":
			{
				...
				String rights = String.valueOf(cluster.isAdded(player.getUUID()));
