		...
		if (args.length > 0)
		{
			...
			switch (arg)
			{
				...
				case "calibrate-analysis":
					...
					try
					{
						threshold = Integer.parseInt(args[1]) / 100.0d;
					}
					...
					PlotAnalysis.calcOptimalModifiers(() -> MainUtil.sendMessage(player, "$1Thank you for calibrating PlotSquared plot expiry"), threshold);
					return true;
				case "stop-expire":
				...
				case "start-rgar":
					...
					boolean result;
					result = HybridUtils.regions != null ? HybridUtils.manager.scheduleRoadUpdate(area, HybridUtils.regions, 0) : HybridUtils.manager.scheduleRoadUpdate(area, 0);
					if (!result)
					{
				...
				case "start-expire":
					...
					return ExpireManager.IMP.runAutomatedTask() ? MainUtil.sendMessage(player, "Started plot expiry task") : MainUtil.sendMessage(player, "Plot expiry task already started");
				case "seen":
					if (args.length != 2)
					{
				...
				case "addcmd":
					try
					{
						String cmd = StringMan.join(Files
				...
				case "list-scripts":
					...
					this.paginate(player, allFiles, 8, page, new RunnableVal3<Integer, File, PlotMessage>() {
					...
					TaskManager.runTaskAsync(() ->
											 {
												 ...
												 try
												 {
													 result = this.engine.eval(toExec, this.scope);
												 }
												 catch (ScriptException e)
												 {
													 e.printStackTrace();
