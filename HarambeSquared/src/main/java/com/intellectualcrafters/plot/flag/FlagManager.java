...
public final class FlagManager {
	...
	static
	{
		...
		try
		{
			MUTABLE_OPTIONAL_FIELD = MUTABLE_OPTIONAL.getClass().getDeclaredField("value");
	...
	{
		V value = getPlotFlagRaw(plot, key);
		if (value != null)
		{
	...
	public static String toString(HashMap<Flag<?>, Object> flags)
	{
		...
		for (Map.Entry<Flag<?>, Object> entry : flags.entrySet())
		{
			try
			{
				...
				flag_string.append(flag.getName()).append(':').append(flag.valueToString(entry.getValue()).replaceAll(":", "¯").replaceAll(",", "´"));
				i++;
			}
			catch (Exception e)
			{
